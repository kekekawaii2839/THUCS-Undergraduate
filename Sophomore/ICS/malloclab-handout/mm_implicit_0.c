...
#define PACK(size, alloc) ((size) | (alloc)) // 将 size 和 alloc 合并为一个 word
...
static void place(void *bp, size_t asize) {
  ...
  if ((size - asize) >= (2 * DSIZE)) {
    ...
    PUT(HDRP(bp), PACK(asize, 1)); // 必须先放 header , 不然没办法计算正确的 footer 的位置
    DEBUG_PRINTF("  *HDRP(bp) = %d\n", *(HDRP(bp)));
    PUT(FTRP(bp), PACK(asize, 1));
    DEBUG_PRINTF("  bp = %p, alloc? %ld\n", bp, GET_ALLOC(bp));
    PUT(HDRP(NEXT_BLKP(bp)), PACK(size - asize, 0)); // 分割后剩下的未分配块
...
void *mm_realloc(void *ptr, size_t size) {
  ...
  copySize = MIN(GET_SIZE(HDRP(ptr)), GET_SIZE(HDRP(bp)));
